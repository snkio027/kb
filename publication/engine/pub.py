...
POLICY = {
    ...
    "publish": "DISABLED",
    "output_policy": "publication/build/preview/<build-id>/<attempt-id>",
...
def input_snapshot(root_fd, profile, views=None):
    from source_model import snapshot
    return snapshot(root_fd, profile, views, sys.modules[__name__])
...
def git_context(root):
    ...
    if Path(os.fsdecode(git("rev-parse", "--show-toplevel")).strip()).resolve() != root.resolve():
        raise PreparationError("publication root must be the Git repository root")
...
def prepare(root, profile, views=None):
    with root_handle(root) as root_fd:
        source = git_context(root)
        snapshot = input_snapshot(root_fd, profile, views)
        runtime = runtime_identity()
        description = identity(snapshot, source, runtime)
        description["publication"] = json.loads(snapshot["source-set.json"])
        build_id = digest(canonical(description))
        attempt_id = uuid.uuid4().hex
        relative = f"publication/build/preview/{build_id}/{attempt_id}"
        outcome = {"status": "PREPARED", "build_id": build_id, "path": str(root / relative)}
        with directory(root_fd, f"publication/build/preview/{build_id}", create=True) as build_fd:
            ...
            with directory(build_fd, attempt_id) as run_fd:
                ...
                try:
                    ...
                    if input_snapshot(root_fd, profile, views) != snapshot or git_context(root) != source:
...
def main(argv=None):
    ...
    parser.add_argument("--view", action="append", help="Repeat to select views; default is every catalog view")
    parser.add_argument("--from-preview", help="repository-relative publication/build/preview/<id>/<attempt>; candidate only")
    ...
    try:
        if args.operation == "candidate":
            ...
            import candidate
            print(json.dumps(candidate.freeze(ROOT, args.from_preview, sys.modules[__name__], args.profile), ensure_ascii=False))
            return 0
        if not args.prepare_only:
            sys.dont_write_bytecode = True
            import preview
            preview.preflight()
        prepared = prepare(ROOT, args.profile, args.view)
