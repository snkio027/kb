...
def command(args, cwd, log=None, input_data=None):
    with subprocess.Popen(list(map(str, args)), cwd=cwd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.PIPE if input_data is not None else None) as child:
        if log:
            dump(str(log) + ".command.json", {"argv": list(map(str, args)), "pid": child.pid, "started": time.time()})
        output, _ = child.communicate(input_data)
    ...
    for doc in selected:
        ...
        for item in walk(body):
            ...
            if tag == "Header":
                ...
                if any(re.search(pattern, text(item)) for pattern in profile.get("new_page_heading_patterns", [])):
                    item["c"][1][2].append(["preview-new-page", "true"])
    ...
    if index:
        ...
        for doc in selected:
            ...
            for header in headers:
                blocks.append(raw(r"\PreviewIndexEntry{" + doc["anchors"][header["c"][1][0]] + "}{" + escape(text(header)) + "}"))
    return blocks, ledger


def build_view(view_config, documents, work, inputs, tools, record, profile, origin):
    import pub
    from links import Resolver
    view = view_config["id"]
    selected = [next(d for d in documents if d["id"] == identity) for identity in view_config["documents"]]
    combined = len(selected) > 1
    meta = dict(selected[0]["meta"]) if not combined else dict(view_config)
    ...
    resolver = Resolver(documents, selected, profile["repository"], origin, tools, pub, command, work, walk)
    blocks, ledger = compose(documents, selected, view_config, profile, resolver, policies)
    ...
    command([tools["pandoc"], folder / "composed.json", "-f", "json", "-t", "latex", "--standalone", "--top-level-division=chapter",
             "--no-highlight", "--wrap=none", "--template", profile_path / "template.tex", "--lua-filter", inputs / "publication/engine/filters/blocks.lua", "-o", folder / "document.tex"], work, folder / "pandoc.log")
    ...
    scoped = section_audit(reader, selected, ledger, ignored_labels, profile, combined)
    ...
    if combined and profile.get("index"):
        identity_changes[reader.get_destination_page_number(names[profile["index"]["id"]])] = meta
    expected = meta
    ...
    audit["source_identity"] = [d["source"] for d in selected]
    audit["profile"] = profile["id"]
    audit["reference_classes"] = {kind: sum(r["kind"] == kind for r in resolver.ledger) for kind in ("EXTERNAL_URI", "EXTERNAL_SOURCE", "SAME_VIEW_INTERNAL")}
    audit["semantic_objects"] = [dict(document=d["id"], **item) for d in selected for item in d["semantic"]]
    audit["explicit_anchors_checked"] = sum(len(d["aliases"]) for d in selected)
    for d in selected:
        for alias, heading in d["aliases"].items():
            if alias not in names or names[alias]["/Page"] != names[d["anchors"][heading]]["/Page"] or names[alias].get("/Top") != names[d["anchors"][heading]].get("/Top"):
                raise RuntimeError("source alias destination mismatch: " + alias)
    dump(folder / "reference-map.json", resolver.ledger)
...
def worker(run, origin):
    ...
    model = record["identity"]["publication"]
    profile_path = inputs / model["profile_path"]
    profile = json.loads((profile_path / "profile.json").read_text())
    spec = importlib.util.spec_from_file_location("frozen_product_adapter", profile_path / "adapter.py")
    adapter = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(adapter)
    os.environ.update(TEXMFVAR=str(work / "cache"), TEXMFCACHE=str(work / "cache"),
                      TEXINPUTS=str(inputs / "publication/latex") + "//:" + str(profile_path) + "//:",
    ...
    for definition in model["sources"]:
        ...
        for pattern in profile.get("required_headings", []):
            if not any(pattern in text(h) for h in headers):
                raise RuntimeError("required product heading absent: " + pattern)
        doc_id = definition["id"]
        anchors = {item: anchor(doc_id, item) for item in ids}
        if not set(aliases.values()) <= set(ids):
            raise RuntimeError("source alias has no owning heading")
        source_href = profile["repository"] + "/blob/" + definition["commit"] + "/" + quote(definition["path"]) if definition["matches_commit"] else "../source/" + quote(definition["path"])
        target = work / "output/source" / definition["path"]
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copyfile(source, target)
        dump(work / "source-ast" / (doc_id + ".json"), ast)
        documents.append({"path": source, "relative_path": definition["path"], "ast": ast, "meta": meta,
                          "id": doc_id, "source": definition, "anchors": anchors, "aliases": aliases,
                          "semantic": semantic, "first_anchor": anchors[ids[0]], "source_href": source_href,
                          "source_matches_commit": definition["matches_commit"]})
    dump(work / "source-catalog.json", [{"source": d["source"], "metadata": d["meta"], "anchors": d["anchors"],
                                      "aliases": d["aliases"], "semantic": d["semantic"], "source_href": d["source_href"]} for d in documents])
    views = [v for v in profile["views"] if v["id"] in model["views"]]
    with ThreadPoolExecutor(max_workers=2) as pool:
        audits = list(pool.map(lambda view: build_view(view, documents, work, inputs, tools, record, profile, origin), views))
    dependencies = {}
    ...
    index = "# " + profile["id"] + " · PDF 阅读预览\n\nPREVIEW，不是发布批准；内容状态与制品状态分开。\n\n"
    for a in audits:
...
def run_preview(prepared, origin, pub):
    ...
    from source_model import verify_live_inputs
    verify_live_inputs(run, origin, pub)
    with pub.root_handle(run) as fd:
        ...
        argv = ["/usr/bin/sandbox-exec", "-p", profile, sys.executable, "-B", str(run / "inputs/publication/engine/preview.py"), "--worker", str(run), str(origin)]
