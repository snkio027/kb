...
class CandidateIsolationTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.relative = 'publication/build/preview/' + 'a' * 64 + '/' + 'b' * 32
        (self.root / self.relative).mkdir(parents=True)
        self.files = {'output/pdf/test.pdf': b'fixture', 'output/source/test.md': b'raw source'}
        self.source = {'build_id': 'a' * 64, 'attempt_id': 'b' * 32, 'identity': {'source': {'commit': 'c' * 40, 'dirty': True}, 'publication': {'profile': 'fixture'}}}
    ...
    def freeze(self, side_effect=None):
        with mock.patch.object(candidate, 'payload', return_value=(self.files, self.source, self.ready), side_effect=side_effect):
            return candidate.freeze(self.root, self.relative, pub, 'fixture')

    def terminals(self):
        return [json.loads(p.read_text()) for p in (self.root / 'publication/build/candidate').glob('*/*/candidate-result.json')]
    ...
    def test_candidate_directory_symlink_rejected(self):
        (self.root / 'publication/build/candidate').symlink_to(self.dist, target_is_directory=True)
        with self.assertRaises(OSError):
            self.freeze()

    def test_preview_directory_symlink_rejected(self):
        linked = self.root / ('publication/build/preview/' + 'e' * 64)
        linked.symlink_to(self.dist, target_is_directory=True)
        with self.assertRaises(OSError):
            candidate.freeze(self.root, str(linked.relative_to(self.root)) + '/' + 'b' * 32, pub, 'fixture')
    ...
    def test_unsafe_and_absolute_preview_paths_rejected(self):
        for path in ('../dist', '/tmp/run', self.relative + '/..'):
            with self.assertRaises(pub.PreparationError):
                candidate.freeze(self.root, path, pub, 'fixture')
