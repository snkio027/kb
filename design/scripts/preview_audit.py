...
def section_audit(reader, documents, ledger, ignored):
    ...
    finish = destination_position(reader, "generated-requirement-index") if "generated-requirement-index" in reader.named_destinations else (len(pages), 0)
    ...
    preface = destination_position(reader, "generated-preface")
    _, preface_text, _ = window(pages, preface, (preface[0] + 1, -10000), (), normalize)
    if not preface_text.startswith(normalize("阅读与版本说明")):
        errors.append({"anchor": "generated-preface", "reason": "wrong target region"})
    navigation_errors = []
    outline_targets = {}
    ...
    if not found or normalize(str(found[1]['/Title'])) != normalize('阅读与版本说明'):
        navigation_errors.append({'anchor': 'generated-preface', 'reason': 'preface bookmark points to wrong region'})
    ...
    for number in range(preface[0] + 1, positions[0][0]):
        for ref in reader.pages[number].get('/Annots', []):
            ...
            if normalize('阅读与版本说明') in normalize(line):
                target = annotation.get('/Dest') or annotation.get('/A', {}).get('/D')
                if target == 'generated-preface':
                    preface_links += 1
                else:
                    navigation_errors.append({'anchor': 'generated-preface', 'reason': 'front TOC link does not use the explicit preface target'})
    if not preface_links:
        navigation_errors.append({'anchor': 'generated-preface', 'reason': 'front TOC preface link not verified'})
    index_entries = 0
    if 'generated-requirement-index' in reader.named_destinations:
        _, index_text, _ = window(pages, finish, (len(pages), 0), (), normalize)
        for doc in documents:
            for header in (n for n in walk(doc['ast']['blocks']) if n['t'] == 'Header' and '-REQ-' in text(n)):
                ...
                if normalize(text(header)) + str(page) not in index_text:
                    navigation_errors.append({'anchor': target, 'reason': 'printed requirement index page does not match PDF destination'})
                index_entries += 1
