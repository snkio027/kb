...
def payload(run_fd, pub):
    ...
    expected_sources = {'output/source/' + name for name in expected if '/' not in name and name.endswith('.md')}
    ...
    for name, data in result.items():
        if name.startswith('output/source/') and data != frozen.get('inputs/' + name.removeprefix('output/source/')):
...
def freeze(root, relative, pub):
    if not re.fullmatch(r'build/preview/[0-9a-f]{64}/[0-9a-f]{32}', relative):
        raise pub.PreparationError('--from-preview must be a design-relative build/preview/<id>/<attempt>')
    with pub.root_handle(root) as root_fd, pub.directory(root_fd, relative) as run_fd:
        ...
        target = f'build/candidate/{candidate_id}/{attempt}'
        with pub.directory(root_fd, f'build/candidate/{candidate_id}', create=True) as candidates:
