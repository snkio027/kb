...
class CompositionTests(unittest.TestCase):
    ...
    def test_uri_classification_full_path_and_fragment(self):
        doc = {"path": Path("/inputs/01-core.md"), "anchors": {"s": "anchor-s"}, "first_anchor": "anchor-s"}
        lookup = {"01-core.md": doc}
        rewrite = lambda url: preview.rewrite_link(url, doc, lookup, True, 'a' * 40)
        ...
        self.assertEqual(rewrite('./01-core.md#s'), '#anchor-s')
        self.assertEqual(rewrite('archive/01-core.md#s'), 'https://github.com/snkio027/kb/blob/' + 'a' * 40 + '/design/archive/01-core.md#s')
        self.assertTrue(rewrite('project/plan.md#范围').endswith('/design/project/plan.md#%E8%8C%83%E5%9B%B4'))
...
class RealPreviewTests(unittest.TestCase):
    ...
    def test_real_success_and_compiler_cannot_write_dist(self):
        ...
        with mock.patch.object(preview, 'command', side_effect=AssertionError('candidate must not compile')):
            frozen = candidate.freeze(self.root, relative, pub)
        ...
        with self.assertRaisesRegex(pub.PreparationError, 'digest changed'):
            candidate.freeze(self.root, relative, pub)
