...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\color{noteslate}C++ Systems Track · G0 Native Toolchain \& Machine Boundary}
...
\title{\textbf{\Large C++ Systems Track · G0 Native Toolchain \& Machine Boundary}}
...
\section{Part 0 · 统一主线与世界观}
\subsection{0.1 G0 的根本问题}
...
\end{lstlisting}
\section{Part 1 · 编译流水线与分离编译}
\subsection{1.1 第一原则：C++ Program 不等于 .cpp 文件}
...
\subsection{1.2 Compiler Driver}
...
└── Linker
\end{lstlisting}
\subsection{1.3 Preprocessor 与 Textual Inclusion}
执行 \texttt{clang++ -E main.cpp -o main.ii} 只执行预处理阶段。

核心机制：传统 \texttt{\#include} 是 Textual Inclusion（文本包含），非 Module Import。

\texttt{\#include} ≠ Python \texttt{import} ≠ Rust \texttt{use} ≠ Zig \texttt{@import}。

\subsection{1.4 Translation Unit (TU)}
定义：Translation Unit = 一个 \texttt{.cpp} + 递归 \texttt{\#include} 的内容 + 宏展开等预处理结果。

分离特性：编译器在编译当前 TU 时，并不天然知道其他 TU 的完整实现。

\subsection{1.5 Declaration vs Definition}
Declaration（声明）：\texttt{int add(int, int);} → 接口契约/承诺（Contract / Promise），告诉编译器符号名、参数类型与返回值类型。

Definition（定义）：\texttt{int add(int a, int b) \{ return a + b; \}} → 实体兑现（Implementation / Fulfillment），生成具体的机器码或分配实际存储。

\subsection{1.6 声明即足以编译调用方代码}
\texttt{clang++ -c main.cpp -o main.o} 在仅有 \texttt{add} 声明时即可编译成功。编译器依据声明和目标平台 ABI，已足够确定传参寄存器与返回方式，并生成调用指令。

\subsection{1.7 Header 的工程定位}
Header 是共享 Declaration 的 Source of Truth。

工程实践：\texttt{math.cpp} 必须 \texttt{\#include "math.hpp"}，使编译器在同一个 TU 内尽早发现声明与定义不匹配。

\section{Part 2 · 目标文件与二进制结构}
\subsection{2.1 “Object” 概念辨析}
C++ Object：\texttt{int x = 42;}（语言语义层实体，具有生命周期与类型）。

Object File：\texttt{main.o}（编译器/链接器层面的目标文件，可重定位二进制工件）。

Object Code：面向目标架构的机器指令代码。

\subsection{2.2 Object File (.o) 的内部组成}
...
\subsection{2.4 Segment 与 Section}
Section（节）：编译器与链接器组织二进制内容的细粒度逻辑分类（如 \texttt{\_\_text}, \texttt{\_\_const}, \texttt{\_\_cstring}, \texttt{\_\_data}）。

Segment（段）：操作系统加载器（Loader）与虚拟内存映射的较大权限管理单位（如 \texttt{\_\_TEXT} 为 R-X，\texttt{\_\_DATA} 为 RW-）。
...
\end{lstlisting}
\subsection{2.5 Symbol 与 Undefined Symbol}
...
\subsection{2.6 Relocation（重定位）}
编译单个 TU 时，外部函数的最终运行时虚拟地址未知。编译器会生成占位调用指令，并在 Relocation 表中记录修补位置（Offset）与目标符号。链接器在确定所有符号地址后执行 Patch。

\subsection{2.7 Name Mangling}
为支持重载、命名空间与模板，C++ 编译器将类型信息编码进符号名称：

C++ 源码：\texttt{int add(int, int)}

Itanium ABI 符号：\texttt{\_Z3addii}

\texttt{nm -C}：执行 Demangling，将符号还原为人类可读源码形式。

\section{Part 3 · 静态库、动态库与加载器}
\subsection{3.1 静态库 (.a) 与符号按需抽取}
...
\end{lstlisting}
\subsection{4.3 Calling Convention（以 Apple ARM64 为例）}
参数传递：通用整型/指针使用 \texttt{x0 - x7}（32 位视图为 \texttt{w0 - w7}），浮点使用 \texttt{v0 - v7}。

返回值：放置于 \texttt{x0 / v0}。

指令：\texttt{BL} 跳转并链接（写入返回地址到 LR），\texttt{RET} 读取 LR 返回。

\subsection{4.4 Alignment, Padding 与 Struct Layout}
Alignment：类型起始地址必须满足的对齐边界约束（如 \texttt{alignof(T) == 4} 要求 \texttt{address \% 4 == 0}）。
...
\end{lstlisting}
\subsection{4.5 Source Compatibility ≠ ABI Compatibility}
结构体增加字段或重排，源码重新编译可能完全正常（源码兼容）；但旧二进制若按旧偏移寻址，会导致内存访问错位（ABI 破坏）。

\subsection{4.6 extern "C" 与 Language Linkage}
\texttt{extern "C"} 指定 C language linkage。在常见 ABI/toolchain 上，其外部符号采用 C linkage 的命名约定，而不是普通 C++ 基于重载的 mangling。函数体内部仍然享有完全的 C++ 语言语义（RAII、模板、异常等）。

\subsection{4.7 C ABI 作为跨语言边界}
...
\subsection{4.8 Opaque Handle 与跨边界所有权}
...
Exception 屏障：严禁 C++ 异常穿透 C ABI 边界，边界函数应标记 \texttt{noexcept} 并转换为错误码返回。

\section{Part 5 · 机器运行时：进程、调用栈与寄存器}
\subsection{5.1 Program vs Process 与虚拟地址空间}
Program：磁盘上的静态二进制工件。

Process：运行中的程序实例，拥有独立的虚拟地址空间、线程栈、文件描述符与内核资源。

\subsection{5.2 Stack, Stack Frame 与 Automatic Storage Duration}
...
\subsection{5.3 SP, LR(x30) 与 FP(x29)}
...
low address
\end{lstlisting}
\subsection{5.4 Leaf Functions 与返回地址保存}
...
\hline
\rowcolor{codebg}
\textbf{寄存器} & \textbf{约定类别} & \textbf{主要用途与角色} \\ \hline
x0 - x7 & Caller-Saved & 参数传递 / 返回值传递（低 32 位为 w0 - w7） \\ \hline
\rowcolor{codebg!50} x9 - x15 & Caller-Saved & 临时通用计算寄存器 \\ \hline
x19 - x28 & Callee-Saved & 长期局部变量与跨调用状态保存 \\ \hline
\rowcolor{codebg!50} x29 (FP) & Callee-Saved & 栈帧指针（Frame Pointer） \\ \hline
x30 (LR) & Caller-Saved (特殊) & 链接寄存器（Link Register，保存返回地址） \\ \hline
\rowcolor{codebg!50} SP & 专职寄存器 & 栈指针（Stack Pointer，需保持 16 字节对齐） \\ \hline
v0 - v31 & 混合 & 浮点与 SIMD 向量寄存器（d0-d31 / s0-s31） \\ \hline
\end{tabularx}
\end{table}
\subsection{5.8 Register Spill 与 As-if 规则}
...
\subsection{5.9 Recursion 与 Stack Overflow}
深层递归持续分配栈帧。当 \texttt{调用深度 × 栈帧大小 > 可用栈内存} 时，将触发 Stack Overflow（macOS 下表现为 EXC\_BAD\_ACCESS）。

\subsection{5.10 悬挂指针与未定义行为 (UB)}
\begin{lstlisting}[language=cpp]
int* bad() {
    int x = 42;
...
\section{Part 6 · 构建系统、诊断模型与核心铁律}
\subsection{6.1 构建系统的本质}
CMake、Ninja、Make、Zig Build 的核心是管理 Artifact Derivation Graph（构建工件的有向无环依赖图 DAG）。

\subsection{6.2 原生工具链的四层世界}
\begin{lstlisting}[language=text]
...
\end{lstlisting}
\subsection{6.3 诊断与性能习惯}
诊断分层：不要泛称“编译报错”，先精准定位发生在五层中的哪一层（预处理、编译、链接、加载还是运行）。

性能观察：源码写了变量不等于内存中一定分配；性能调优必须观察汇编、Profiler 与 Cache 行为。

\subsection{6.4 G0 自测清单}
\begin{itemize}
  \item 能否清晰解释 .cpp 到 CPU 执行的完整阶段与各个中间工件？
  \item 为什么只有 Declaration 也能成功生成包含调用的 .o 文件？
  \item Relocation 到底解决了单文件编译时的什么核心问题？
  \item .a 静态库与 .dylib 动态库在链接和加载时有何本质区别？
  \item 为什么 public struct 调整字段顺序属于破坏 ABI？
  \item ARM64 的 LR 与 FP 在非叶子函数调用中分别承担什么职责？
  \item 为什么局部变量退出作用域后原地址可能还能读出旧数据，但这依然是严重 UB？
\end{itemize}
\subsection{6.5 十条不可动摇的核心铁律}
\begin{enumerate}
  \item C++ 程序不等于 .cpp：原生程序是 Source → TU → Object → Link → Load → Process 的系统化结果。
  \item Translation Unit 是传统 C++ 分离编译的核心独立单元。
  \item Declaration 提供接口契约，Definition 真正提供实体实现。
  \item Object File 是结构化的二进制工件，包含机器码、Sections、符号表与重定位记录。
  \item Linker 的核心职责 是符号解析（Symbol Resolution）与地址重定位（Relocation）。
  \item 静态库是 .o 归档包；动态库是独立的运行时镜像。
  \item API 是源码级调用契约，ABI 是机器码级协作契约。
  \item 源码变量、对象生命周期与物理内存栈槽是完全解耦的概念。
  \item 函数调用在底层统一落地为寄存器传参、栈对齐、返回地址保存与 ABI 规范。
  \item 源码仅描述程序语义，不等于机器执行模型；系统级分析必须能够下沉至二进制与硬件层。
\end{enumerate}
\end{document}